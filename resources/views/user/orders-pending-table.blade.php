            ...
            <tbody>
                @foreach($orders as $key => $order)
                    @if (strtolower($order['confirmed']) == $status)
                            <tr class="order-pending-details"
                                data-id="{{$order['id']}}"
								data-status="{{$status}}"
								data-url="{{ url(App::getLocale() . '/order/' . $order['id']) }}"
                                data-bs-toggle="modal" data-bs-target="#orderPendingDetailsModal" style="cursor: pointer">
...
<div class="modal fade" id="orderPendingDetailsModal" tabindex="-1" aria-labelledby="orderPendingDetailsModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="model-content-box">
				<div class="modal-header">
					<h5 class="modal-title" id="orderPendingDetailsModalLabel"></h5>
					<button type="button" class="btn-close close {{$ml_mr_auto}}" data-bs-dismiss="modal" aria-label="Close">
{{--						<span aria-hidden="true">&times;</span>--}}
					</button>
				</div>
				<div class="modal-body modal-ajax" style="background: #f5f5f5">
...
@push('js')
<script>
    $('.tab-content').on('click', '.order-pending-details', function () {
        ...
        $.ajax({
            ...
            data: formData,
            beforeSend: function () {
                $('#orderPendingDetailsModal .model-loader').removeClass('d-none');
                $('#orderPendingDetailsModal .model-message').addClass('d-none');
                $('#orderPendingDetailsModal .model-content-box').addClass('d-none');
            },
            ...
            success: function (response, status) {
                if (status == 'success') {
                    ...
                    // $('#orderPendingDetailsModal').find('.modal-body').html(JSON.parse(response.data));
                    $('#orderPendingDetailsModal').find('.modal-ajax').html(response.view);
