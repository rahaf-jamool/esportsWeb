                        ...
                        <div class="row w-100 h-100 align-items-center mx-auto">
                            <div class="col-4 h-100">
                                @if (!empty($user['orgnizationInfo']['imagePath']))
                                    <img class="responsive" id="original_card_image" src="{{ config('app.base_address') . $user['orgnizationInfo']['imagePath'] }}" alt="">
                                @else
                                    <img class="img-fluid" id="card_image" src="{{ asset('SD08/default-user-image.png') }}" alt="">
                                @endif
                            </div>
                            <div class="col-8 pl-0">
                                <div class="cart-content-info">
                                    <ul class="list-unstyled">
                                        <li>
                                            {{trans('auth.name')}} : <span id="card_firstName">{{ isset($user['client']['name']) && !is_null($user['client']['name']) ? $user['client']['name'] : '----' }}</span>
                                        </li>
{{--                                        <li>{{trans('individually.date-birth')}} : <span id="card_BirthDate">----</span></li>--}}
                                        <li>{{trans('auth.account-type')}} : <span
                                                id="card_account-type">{{ isset($user['client']['type']) && !is_null($user['client']['type']) ? $user['client']['type'] : '----' }}</span></li>
{{--                                        <li>{{trans('individually.nationality')}} : <span id="card_NationalityId">----</span></li>--}}
                                        <li>{{trans('individually.emirates-number')}} : <span
                                                id="card_uaeIdNumber">{{ isset($user['orgnizationInfo']['membershipNumber']) && !is_null($user['orgnizationInfo']['membershipNumber']) ? $user['orgnizationInfo']['membershipNumber'] : '----' }}</span></li>
                                        <li>{{trans('individually.emirates-expiry-date')}} : <span
                                                id="card_uaeIdEndDate">{{ isset($user['membershipEndDate']) && !is_null($user['membershipEndDate']) ? \Carbon\Carbon::parse($user['membershipEndDate'])->format('Y-m-d') : '----' }}</span></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                ...
                <div class="download-link text-center my-3">
                    ...
                    <a href="javascript:void(0)" id="card_canv" class="btn btn-outline-success">{{ trans('site.download-as-pdf') }}</a>
                </div>
                <div id="img_out" class="d-none"><img src="" /></div>
            </div>
