...
{{--                <div class="col-6">--}}
{{--                    <p>--}}
{{--                        {{ !is_null($team['teamHeadName']) ? $team['teamHeadName'] : '-------'}}--}}
{{--                    </p>--}}
{{--                </div>--}}
{{--            </div>--}}
            <div class="col-12 col-lg-6 m-2 border-bottom mx-auto p-2 d-flex justify-content-between">
            ...
            @if (!empty($team['clubName']))
                <div class="col-12 col-lg-6 m-2 border-bottom mx-auto p-2 d-flex justify-content-between">
                    ...
                    <div class="col-6">
                        <p>
                            {{ !is_null($team['clubName'])?$team['clubName']:''}}
                        </p>
                    </div>
                </div>
            @endif
            ...
            <div class="col-12">
                ...
                <div class="alert alert-danger text-center club-team-error-message mb-3 d-none"></div>
                <div class="alert alert-success text-center club-team-success-message mb-3 d-none"></div>
                @if (!$team['accepted'])
                    <div class="text-left">
                        <button type="button" id="clubAddNewPlayerButton" class="btn btn-outline-dark btn-sm mb-3" data-toggle="modal" data-target="#custom-message">{{ trans('site.add-player-not-in-club') }}</button>
                        <button type="button" id="clubAddPlayerFromClubButtonPending" class="btn btn-outline-dark btn-sm mb-3" data-toggle="modal" data-target="#custom-message">{{ trans('site.add-player-in-club') }}</button>
                    </div>
                @else
                    <div class="text-left">
                        <button type="button" id="clubAddNewPlayerButton" class="btn btn-outline-dark btn-sm mb-3" data-toggle="modal" data-target="#clubAddNewPlayerModel" data-teamid="{{ $team['id'] }}">{{ trans('site.add-player-not-in-club') }}</button>
                        <button type="button" id="clubAddPlayerFromClubButton" class="btn btn-outline-dark btn-sm mb-3" data-teamid="{{ $team['id'] }}">{{ trans('site.add-player-in-club') }}</button>
                    </div>
                @endif
                        ...
                        @if (count($team['players']) > 0)
                            @foreach($team['players'] as $member)
                                <tr>
                                    ...
                                    <td style="text-align:center;vertical-align: middle"><a class="text-danger club-remove-player" type="button" data-teamid="{{ $team['id'] }}" data-id="{{ $member['id'] }}" href="javascript:void(0)">{{ trans('individually.remove') }}</a></td>
                                </tr>
                            @endforeach
                        @else
                            <tr>
