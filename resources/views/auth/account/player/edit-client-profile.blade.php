		...
		<div class="container" style="padding-top: 100px;">
            ...
            <form class="form-horizontal" role="form" method="POST" action="{{ url(App::getLocale() . '/profile/edit/' . $user['id']) }}" enctype="multipart/form-data">
                    ...
                    <div class="form-group {{ $errors->has('gender') ? ' has-error' : '' }}">
                        ...
                        <div class="col-sm-12 radio-btn">
                            <label class="radio-inline">
                                <input type="radio" class="mx-2" name="gender" value="M"  {{ ($user['personInfo']['gender']=="M")? "checked" : "" }} style="">{{trans('individually.male')}}
                            </label>
                            <label class="radio-inline">
                                <input type="radio" class="mx-2" name="gender" value="F"  {{ ($user['personInfo']['gender']=="F")? "checked" : "" }}>{{trans('individually.female')}}
                            </label>
                    ...
                    <div class="form-group {{ $errors->has('socialStatus') ? ' has-error' : '' }}">
                        ...
                        <div class="col-sm-12 radio-btn">
                            <label class="radio-inline">
                                <input type="radio" class="mx-2" name="socialStatus" value="Married" {{ ($user['personInfo']['socialStatus']=="Married")? "checked" : "" }} style="">{{trans('individually.married')}}
                            </label>
                            <label class="radio-inline">
                                <input type="radio" class="mx-2" name="socialStatus" value="Single" {{ ($user['personInfo']['socialStatus']=="Single")? "checked" : "" }}>{{trans('individually.single')}}
                            </label>
                            ...
                            @if (!empty($educationLevel))
                                @foreach($educationLevel as $level)
                                    <option value="{{ $level['id'] }}"
                                            style="direction: {{$rtl}}" {{ $level['id'] == $user['personInfo']['educationLevelId'] ? 'selected' : '' }}>{{ getTranslate($level, 'name') }}</option>
                                @endforeach
                            @endif
                        </select>
                    ...
                    <div class="form-group {{ $errors->has('uaeResidency') ? ' has-error' : '' }}">
                        ...
                        <div class="col-sm-12 radio-btn">
                            <label class="radio-inline jcf-label-active">
                                {{ trans('individually.yes') }}<input type="radio" class="mx-2" name="uaeResidency" {{ ($user['personInfo']['uaeResidency']== true)? "checked" : "" }} value="true">
                            </label>
                            <label class="radio-inline">
                                {{ trans('individually.no') }}<input type="radio" class="mx-2" name="uaeResidency" {{ ($user['personInfo']['uaeResidency']== false )? "checked" : "" }} value="false">
                            </label>
                                ...
                                @foreach($platforms as $platform)
                                    <div class="custom-control custom-switch">
                                        <input type="checkbox" class="custom-control-input" id="playerPlatforms_{{$platform['id']}}"
                                               ...
                                               @foreach ( $user['playerPlatforms'] as  $val)
                                ...
                                @foreach($games as $game)
                                    <div class="custom-control custom-switch">
                                        <input type="checkbox" class="custom-control-input" id="playerGames_{{$game['id']}}"
                                               ...
                                               @foreach ( $user['playerGames'] as  $val)
