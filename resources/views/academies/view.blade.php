...
@section('content')
    ...
    @if (isset($academy) && !is_null($academy))
    <!-- End header -->
    <section class="details-services">
        <div class="club">
                <div class="title-club-image">
                    @if(count($academy['orgnizationInfo']) > 0)
                        <div class="image-details d-flex" style="width: 100px;">
                            <img class="responsive" src="{{ config('app.base_address') . $academy['orgnizationInfo']['imagePath'] }}" alt="alt image" onerror="this.onerror=null;this.src='{{asset('assets/img/logo1.png')}}';this.style='width:100%';"/>
                        </div>
                    @else
                        <div class="image-details" style="width:200px">
                ...
                <div class="card">
                    <h4>{{trans('site.academies')}}</h4>
                    <div class="information">
                        <div class="info-player">
                            <p>{{trans('auth.email')}}</p><span>{{$academy['orgnizationInfo']['email']}}</span>
                        </div>
                        <div class="info-player">
                            <p>{{trans('auth.phone')}}</p><span>{{$academy['orgnizationInfo']['phone']}}}</span>
                        </div>
                        <div class="info-player">
                            <p>{{trans('auth.address')}}</p><span>{{$academy['orgnizationInfo']['address']}}</span>
                        </div>
                        <div class="info-player">
                            <p>{{trans('institutions.licensedate')}}</p><span>{{ \Carbon\Carbon::parse($academy['orgnizationInfo']['licenceEndDate'])->format('d/m/Y')}}</span>
                        </div>
                    </div>
                </div>
                @if (count($players) > 0)
