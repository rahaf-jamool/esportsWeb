    ...
    <div class="about-header" >
        ...
        <div class="grid-container py-3 py-md-5">
            <div class="grid-x grid-margin-x small-up-1 medium-up-2 large-up-4 grid-x-wrapper" data-aos="fade-up"
                ...
                @if (isset($NationalTeams) && count($NationalTeams) > 0)
                    @foreach ($NationalTeams as $item)
                        <div class="product-box column" data-aos="fade-up" data-aos-duration="2000">
                            <a href="{{url(App::getLocale() . '/national-teams/' . $item['id'] . '/view')}}" class="product-item p-0">
                                @if(!empty($item['groupImagePath']))
                                    <div class="product-item-image">
                                        <img src="{{ config('app.base_address') . $item['groupImagePath']}}" alt="alt image" onerror="this.onerror=null;this.src='{{asset('assets/img/logo1.png')}}';"/>
                                    </div>
                                @else
                                    <div class="product-item-image">
